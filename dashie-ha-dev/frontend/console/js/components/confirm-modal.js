...
const ConfirmModal = {
    ...
    /**
     * Native prompt() replacement. Resolves the trimmed string, or null on
     * cancel / Esc / backdrop.
     *
     * 🔴 Returns null for "cancelled" and never '' — an empty string is a value
     * a caller could legitimately store, so collapsing the two would make a
     * cancelled rename indistinguishable from a rename to nothing.
     *
     * @param {Object} opts
     * @param {string} opts.title
     * @param {string} [opts.label]        - hint above the input
     * @param {string} [opts.value='']     - prefilled text (selected on open)
     * @param {string} [opts.placeholder]
     * @param {string} [opts.confirmLabel='Save']
     * @param {boolean} [opts.required=true] - Confirm stays disabled while empty
    ...
    _render({
        ...
        textInput = null,     // {label, value, placeholder, required} — prompt() mode
    }) {
        ...
        // prompt() mode. Shares this dialog's markup, focus handling, Esc/backdrop
        // and cleanup rather than standing up a second modal -- the seam rule: one
        // dialog implementation, two entry points.
        const isPrompt = !!textInput;
        const promptId = 'confirm-prompt-' + Math.random().toString(36).slice(2, 8);
        const promptSection = isPrompt
            ? `<div style="margin-bottom: 16px;">
                   ${textInput.label ? `<label for="${promptId}" style="display: block; font-size: 13px; color: var(--text-secondary); margin-bottom: 6px;">${this._escape(textInput.label)}</label>` : ''}
                   <input id="${promptId}" type="text" autocomplete="off" autocapitalize="sentences"
                          spellcheck="false" placeholder="${this._escape(textInput.placeholder || '')}"
                          value="${this._escape(textInput.value || '')}"
        ...
        root.innerHTML = `
            <div role="dialog" aria-modal="true" aria-labelledby="confirm-title" style="background: var(--bg-card, #fff); border-radius: 12px; max-width: 460px; width: 100%; box-shadow: 0 20px 60px rgba(0,0,0,0.3); padding: 20px;">
                ...
                ${typedSection}${promptSection}
                <div style="display: flex; gap: 8px; justify-content: flex-end;">
                    ${hideCancel ? '' : `<button class="btn btn-ghost" data-confirm-action="cancel">${this._escape(cancelLabel)}</button>`}
                    <button class="${confirmClass}" data-confirm-action="confirm" ${(hasTypedGate || (isPrompt && textInput.required && !String(textInput.value || '').trim())) ? 'disabled' : ''}>${this._escape(confirmLabel)}</button>
                </div>
            </div>
        `;
        // Backdrop click cancels (only on the root, not when clicking inside the dialog)
        root.addEventListener('click', e => {
            if (e.target === root) this._answer(isPrompt ? null : false);
        });
        ...
        cancelBtn?.addEventListener('click', () => this._answer(isPrompt ? null : false));
        const promptValue = () => (root.querySelector('#' + promptId)?.value || '').trim();
        const promptOk = () => !isPrompt || !textInput.required || !!promptValue();
        confirmBtn.addEventListener('click', () => {
            ...
            if (isPrompt) { if (promptOk()) this._answer(promptValue()); return; }
            this._answer(true);
        });
        if (isPrompt) {
            const input = root.querySelector('#' + promptId);
            input.addEventListener('input', () => { confirmBtn.disabled = !promptOk(); });
        }
        ...
        this._onKeyDown = e => {
            // 🔴 Cancel resolves null in prompt mode, never ''. '' is a value a caller
            // could store; null is the only unambiguous "the user backed out".
            if (e.key === 'Escape') { e.preventDefault(); this._answer(isPrompt ? null : false); }
            else if (e.key === 'Enter') {
                e.preventDefault();
                if (isPrompt) { if (promptOk()) this._answer(promptValue()); return; }
        ...
        setTimeout(() => {
            if (isPrompt) {
                const el = root.querySelector('#' + promptId);
                el?.focus();
                el?.select();   // a prefilled rename/duplicate name is replaceable at once
            } else if (hasTypedGate) root.querySelector('#' + inputId)?.focus();
